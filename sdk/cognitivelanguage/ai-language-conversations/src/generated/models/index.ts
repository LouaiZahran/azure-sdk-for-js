...
export interface BaseExtraInformation {
  /** Polymorphic discriminator, which specifies the different types this object can be */
  extraInformationKind: "EntitySubtype" | "ListKey" | "RegexKey";
}
...
export interface SummaryResultItem {
  ...
  contexts?: ItemizedSummaryContext[];
}
...
/** The result from sentiment analysis operation for each conversation item. */
export interface ConversationSentimentResult {
  /** Enumeration of Sentiment operation results for all the conversation items in a conversation. */
  conversationItems: ConversationSentimentItemResult[];
}

export interface ConversationSentimentItemResult {
  /** The identifier for the conversation item */
  id: string;
...
export interface AnalyzeConversationLROTask extends TaskIdentifier {
  /** Enumeration of supported analysis tasks on a collection of conversation. */
...
export interface TranscriptConversationItem extends ConversationItemBase {
  ...
  wordLevelTimings?: WordLevelTiming[];
  /** Conversation item level audio timing. This still can help on AI quality if word level audio timings are not available. */
...
export interface RegexKey extends BaseExtraInformation {
  /** Polymorphic discriminator, which specifies the different types this object can be */
...
/** The conversation item level audio timing. */
export interface ConversationItemLevelTiming extends AudioTiming {}
...
export interface ConversationSentimentResultsConversationsItem
  extends ConversationSentimentResult,
    ConversationResultBase {}
...
/** The result from sentiment analysis operation for each conversation. */
export interface ConversationSentimentResults extends PreBuiltResult {
  conversations: ConversationSentimentResultsConversationsItem[];
}
...
/** The context of the summary with conversation item id. */
export interface ItemizedSummaryContext extends SummaryContext {
...
/** Task definition for a sentiment analysis in conversations. */
export interface AnalyzeConversationalSentimentTask
  extends AnalyzeConversationLROTask {
  /** Supported parameters for a Conversational sentiment analysis task. */
...
/** Result from the sentiment analysis operation performed on a list of conversations. */
...
/** Supported parameters for a Conversational sentiment analysis task. */
export interface ConversationalSentimentTaskParameters
  extends PreBuiltTaskParameters {
  /** For transcript conversations, this parameter provides information regarding which content type should be used for sentiment analysis. The details of the sentiment analysis - like the offset, length and the text itself - will correspond to the text type selected here. */
...
export enum KnownSummaryAspect {
  /** A summary of issues in transcripts of web chats and service call transcripts between customer-service agents, and your customers. */
  Issue = "issue",
  /** A summary of resolutions in transcripts of web chats and service call transcripts between customer-service agents, and your customers. */
  Resolution = "resolution",
  /** A chapter title of any conversation. It is usually one phrase or several phrases naturally combined together. Long conversation tends to have more chapters. The chapter boundary can be found from the summary context. */
  ChapterTitle = "chapterTitle",
  /** A generic narrative summary of any conversation. It generally converts the conversational language into formal written language, compresses the text length and keeps the salient information. */
  Narrative = "narrative"
}
 ...
 *  this enum contains the known values that the service supports.
 * ### Known values supported by the service
 * **issue**: A summary of issues in transcripts of web chats and service call transcripts between customer-service agents, and your customers. \
 * **resolution**: A summary of resolutions in transcripts of web chats and service call transcripts between customer-service agents, and your customers. \
 * **chapterTitle**: A chapter title of any conversation. It is usually one phrase or several phrases naturally combined together. Long conversation tends to have more chapters. The chapter boundary can be found from the summary context. \
 * **narrative**: A generic narrative summary of any conversation. It generally converts the conversational language into formal written language, compresses the text length and keeps the salient information.
 */
export type SummaryAspect = string;

/** Known values of {@link TextSentiment} that the service accepts. */
export enum KnownTextSentiment {
  /** Positive sentiment. */
  Positive = "positive",
  /** Neutral sentiment. */
  Neutral = "neutral",
 ...
 * {@link KnownTextSentiment} can be used interchangeably with TextSentiment,
 *  this enum contains the known values that the service supports.
 * ### Known values supported by the service
 ...
 */
export type TextSentiment = string;
